...
class BlockSource(TopologySource):
    ...
    traits_view = View('name', 'dims', 'position', 'x_axis', 'z_axis', 'modified')
    ...
    def execute(self):
        ax = gp.gp_Ax2(gp.gp_Pnt(*self.position),  gp.gp_Dir(*self.z_axis), gp.gp_Dir(*self.x_axis))
...
class BooleanOpFilter(FilterSource):
    ...
    map = {'cut': BRepAlgoAPI.BRepAlgoAPI_Cut, 'fuse': BRepAlgoAPI.BRepAlgoAPI_Fuse,
           'common': BRepAlgoAPI.BRepAlgoAPI_Common}
    
    traits_view = View('operation', 'modified')
...
class ChamferFilter(FilterSource):
    ...
    traits_view = View(Item('edge_id'), 'size', 'modified')
